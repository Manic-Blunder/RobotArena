...
class MagentaDood(arena: Arena, genome: Genome, position: Vector2) : Dood(arena, position, Team.MAGENTA, genome) {
...
//            val rays = 30
//            val max = (0 until rays / 2).sum().toDouble()
//            val raysults: ArrayList<Pair<Double, Double>> = arrayListOf()
//            repeat(rays) { i ->
//                var blah = (0 until abs(i - rays / 2)).sum().toDouble()
//                var piBlah = (max - blah) / max * PI
//                if (i - rays / 2 < 0) piBlah *= -1
...
//                raysults.add(raysult)
//                inputs.add(raysult.first)
//                inputs.add(raysult.second)
//            }
//            val nearestAlly = raysults.filter { it.second == -1.0 }.minByOrNull { it.first }
//            if (nearestAlly != null){
//                inputs.add(15.0 - abs(15.0 - raysults.indexOf(nearestAlly)))
//                inputs.add(nearestAlly.first)
//            } else {
//                inputs.add(0.0)
//                inputs.add(0.0)
//            }
//            val nearestWall = raysults.filter { it.second == -0.5 }.minByOrNull { it.first }
//            if (nearestWall != null){
//                inputs.add(15.0 - abs(15.0 - raysults.indexOf(nearestWall)))
//                inputs.add(nearestWall.first)
//            } else {
//                inputs.add(0.0)
//                inputs.add(0.0)
//            }
//            val nearestEnemy = raysults.filter { it.second == 1.0 }.minByOrNull { it.first }
//            if (nearestEnemy != null){
//                inputs.add(15.0 - abs(15.0 - raysults.indexOf(nearestEnemy)))
//                inputs.add(nearestEnemy.first)
//            } else {
//                inputs.add(0.0)
//                inputs.add(0.0)
//            }
    ...
    override fun processNeuralOutputs(outputs: List<Double>) {
        if (outputs[0] > 0.5) commands.add(Command.MOVE_FORWARD)
        if (outputs[1] > 0.5) commands.add(Command.MOVE_BACK)

        if (outputs[2] > 0.5) commands.add(Command.MOVE_RIGHT)
        if (outputs[3] > 0.5) commands.add(Command.MOVE_LEFT)

        if (outputs[4] > 0.5) commands.add(Command.ROTATE_RIGHT)
        if (outputs[5] > 0.5) commands.add(Command.ROTATE_LEFT)

        if (outputs[6] > 0.5) commands.add(Command.FIRE)
